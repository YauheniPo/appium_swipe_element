...
from appium_swipe_element.constants import *


def get_swipe_params(way, measure_percent, params):
    swipe_prams = {'start_x': None,
                   'start_y': None,
                   'end_x': None,
                   'end_y': None}

    if way == UP_WAY or way == DOWN_WAY:
    ...
    if way == LEFT_WAY or way == RIGHT_WAY:
...
class AppiumHelper(AppiumLibrary):
    ...
    def scroll_element(self,
        ...
        self._browser().swipe(**scrolling_element_params,
                              duration=duration)

        return scrolling_element_params
